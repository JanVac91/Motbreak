bl_info = {
    "name": "Capcom Outbreak Animation Importer (V1.2.9 + LOOP ONLY)",
    "author": "Gemini & User",
    "version": (1, 2, 9),
    "blender": (3, 0, 0),
    "location": "File > Import > Capcom Outbreak Anim (.mot)",
    "description": "V1.2.9: Loop detection added. No cleaning/reset logic.",
    ...
    try:
        with open(filepath, "rb") as f:
            ...
            while current_section_offset + 20 <= file_size:
                ...
                h_data = f.read(20)
                # h_loop (int) and h_loopFrame (float)
                ...
                for n in range(h_count):
                    ...
                    if n_sub > 0:
                        if target: target.rotation_mode = 'XYZ'
...
class IMPORT_OT_capcom_outbreak_summary(bpy.types.Operator, ImportHelper):
    ...
    def execute(self, context):
        ...
        if success and loops:
            # Show a popup in Blender UI if loops are found
            bpy.context.window_manager.popup_menu(lambda self, context: [self.layout.label(text=m) for m in loops], title="Loop Information", icon='LOOP_FORWARDS')
        return {'FINISHED'}
